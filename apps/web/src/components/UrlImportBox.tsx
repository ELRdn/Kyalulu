...
export default function UrlImportBox({ onPreview, disabled = false, onBusyChange }: { onPreview: (value: ImportPreview) => void; disabled?: boolean; onBusyChange?: (busy: boolean) => void }) {
  ...
  return <section className="k-url-import" aria-label="URL取り込み"><h2>公開URLから取り込む</h2><p>TavernCard・RisuRealmのコンテンツ、GitHub・Hugging FaceのファイルURLに対応しています。</p>
    <form className="k-url-import" onSubmit={e => { e.preventDefault(); void submit(); }}>
